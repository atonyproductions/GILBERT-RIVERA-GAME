...
var actorChars = {
  ...
  "=": Lava, "u": Lava, "v": Lava, "k": Lava,"l": Lava,
    "e": Enemy,
...
function Enemy(pos) {
    this.basePos = this.pos = pos.plus(new Vector(-1.5, -2));
  ...
  this.repeatPos = pos;
    ...
    this.wobble = Math.random() * Math.PI + 40;
...
function Lava(pos, ch) {
  ...
  if (ch == "=") {
    ...
    // Drip  Repeat back to this pos.
    this.speed = new Vector(0, -4);
    this.size = new Vector(.5,.5)
    this.repeatPos = pos;
  } else if (ch == "k") {
    // go up Repeat back to this pos.
    this.speed = new Vector(0, 4.5);
    this.size = new Vector(.5,.5)
    this.repeatPos = pos;
  }
}
...
function runGame(plans, Display) {
  ...
  var audio = new Audio('elecman.mp3');
  audio.play();
  ...
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lost")
		...
		img.src = "http://i.imgur.com/nQaGjCl.png";
